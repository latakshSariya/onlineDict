...
import { Text, View, StyleSheet, TouchableOpacity, Button, TextInputComponent, TextInput, Image, KeyboardAvoidingView, Alert } from 'react-native';
...
export default class issuescreen extends React.Component {
  ...
  checkStudentEligiblityForBookIssue = async()=>{
    ...
    if(studentRef.docs.length == 0){
      ...
      console.log("studentEligiblit "+studentEligiblity);
      alert("The student id does not exixt in the database")
    }else{
      studentRef.docs.map(doc=>{
        var student = doc.data()
        console.log("bookIssueValue ", student.BooksIssued)
  ...
  handleTransation=async()=>{
    ...
    console.log("TransationType"+TransationType)
    if(!TransationType){
      ...
      console.log("studentEligiblity"+studentEligiblity)
    ...
    if (buttonState !== 'normal' && CameraPermission) {
      ...
      return (
        <KeyboardAvoidingView style={{flex:1, justifyContent:'center', alignContent:'center'}} behavior = "padding" enabled>
          ...
          <Text>{this.state.transation}</Text>
